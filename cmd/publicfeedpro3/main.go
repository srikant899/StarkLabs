...
type yOptionResp struct { OptionChain struct { Result []struct { Options []struct { Calls []optionLite `json:"calls"`; Puts []optionLite `json:"puts"` } `json:"options"` } `json:"result"` } `json:"optionChain"` }
...
func main() {
  ...
  for _, s := range []string{"NVDA","META","TSLA","AAPL","MSFT"} {
    ...
    } else if fallback, sp := modeledFallback(s, feed.Stocks); fallback.Symbol != "" {
  ...
  _ = os.WriteFile("docs/public-feed-pro-v3.json", b, 0644)
  fmt.Println("generated public-feed-pro-v3.json")
}
...
func fetchOptionAndSpread(symbol string) (OptionCard, SpreadCard, error) {
  ...
  if len(opt.Calls) > 0 {
    ...
    buy := round(c.LastPrice * 0.96)
    tgt := round(c.LastPrice * 1.22)
    stop := round(buy * 0.82)
    pl := round((tgt-buy)*1)
    return OptionCard{Symbol:c.ContractSymbol, Underlying:symbol, Strategy:fmt.Sprintf("Bull call setup near %.0f strike", c.Strike), Price:round(c.LastPrice), BuyPrice:buy, Target:tgt, Quantity:1, ProfitLoss:pl, StopLoss:stop, Confidence:73, Source:"yahoo_public", UpdatedAt:time.Now().UTC().Format(time.RFC3339)}, SpreadCard{Underlying:symbol, Direction:"bullish", Idea:fmt.Sprintf("Buy %.0fC and sell next higher call", c.Strike), BuyPrice:buy, Debit:round(c.LastPrice), MaxProfit:round((tgt-buy)*100), StopLoss:stop, Confidence:70, Source:"modeled_from_public_chain"}, nil
  }
  if len(opt.Puts) > 0 {
    p := bestPutLite(opt.Puts)
    buy := round(p.LastPrice * 0.96)
    tgt := round(p.LastPrice * 1.20)
    stop := round(buy * 0.82)
    pl := round((tgt-buy)*1)
    return OptionCard{Symbol:p.ContractSymbol, Underlying:symbol, Strategy:fmt.Sprintf("Bear put setup near %.0f strike", p.Strike), Price:round(p.LastPrice), BuyPrice:buy, Target:tgt, Quantity:1, ProfitLoss:pl, StopLoss:stop, Confidence:71, Source:"yahoo_public", UpdatedAt:time.Now().UTC().Format(time.RFC3339)}, SpreadCard{Underlying:symbol, Direction:"bearish", Idea:fmt.Sprintf("Buy %.0fP and sell next lower put", p.Strike), BuyPrice:buy, Debit:round(p.LastPrice), MaxProfit:round((tgt-buy)*100), StopLoss:stop, Confidence:69, Source:"modeled_from_public_chain"}, nil
  }
...
func modeledFallback(symbol string, stocks []StockCard) (OptionCard, SpreadCard) {
  for _, s := range stocks {
    if s.Symbol == symbol {
      ...
      return OptionCard{Symbol:symbol+"-MODELED", Underlying:symbol, Strategy:strat, Price:price, BuyPrice:buy, Target:target, Quantity:1, ProfitLoss:round((target-buy)*1), StopLoss:stop, Confidence:64, Source:"modeled_from_stock_price", UpdatedAt:time.Now().UTC().Format(time.RFC3339)}, SpreadCard{Underlying:symbol, Direction:dir, Idea:idea, BuyPrice:buy, Debit:price, MaxProfit:round((target-buy)*100), StopLoss:stop, Confidence:62, Source:"modeled_from_stock_price"}
    }
  }
...
func buildTargets(price, vol, r20, r60 float64) []TargetRow {
  ...
  for _, h := range horizons {
    ...
    entryBuffer := clamp(vol*math.Sqrt(h.t)*0.35+math.Max(0,-drift)*0.18, 0.01, 0.18)
    buy := round(price * (1-entryBuffer))
    stop := round(math.Max(buy-band*0.55, buy*0.86))
    conf := round(clamp(76-vol*38+h.t*10, 38, 90))
    upside := round((target/buy - 1) * 100)
    out = append(out, TargetRow{Horizon:h.label, BuyPrice:buy, TargetPrice:target, Quantity:20, ProfitLoss:round((target-buy)*20), StopLoss:stop, Confidence:conf, UpsidePct:upside})
  }
  return out
}
